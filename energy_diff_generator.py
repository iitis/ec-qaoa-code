...
ps = [[0.01, 0.005, 0.002, 0.0005][int(sys.argv[7])]]
scheme = "0001"
# use_schem = 'on'
        ...
        for t in range(1, layers_no+1):
            ...
            for a in range(1,instances_no+1):
                ...
                if angles_typ == 'optim':
                    ...
                    en_pure,_,_ = qaoa_xy(angle,nse_md(0),diag_vals,hamiltonian=hamildict)
                ...
                for (k_ind, k) in enumerate(my_keys):
                    ...
                    data_prob[k_ind+1,t-1,a-1] = np.real(data_raw[1])
