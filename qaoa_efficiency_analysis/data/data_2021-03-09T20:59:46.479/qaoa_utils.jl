...
function _state!(hamiltonian::Vector{Float64}, times::Vector{Float64}, tmp_data::Dict, init_state::Vector{ComplexF64})
...
function _fg!(F, gradient, hamiltonian::Vector{Float64}, times, tmp_data::Dict, init_state::Vector{ComplexF64})
    k = div(length(times), 2)
    qubits_no = Int(log2(length(hamiltonian)))
