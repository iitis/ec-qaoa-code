...
for cit, city in enumerate(city_no):
            ...
            for i in range(1,exp_no+1):
                for k_ind in range(len(my_keys)):
                    ...
                    for a in range(instances_no):
                        ...
                        data_en_mean[p,i,k_ind] += (nominator-denominator)/(nominator+denominator)
        ...
        for p in noises:
            for k_ind in range(len(my_keys)):
                if mean_calc == 'std':
                    ...
                    data_en_mean_plot[p,k_ind] = [np.median([data_en_mean[p,i,k_ind][t] for i in range(1,exp_no+1)]) for t in range(layers_no)]
                    data_en_max_plot[p,k_ind] = [np.percentile([data_en_mean[p,i,k_ind][t] for i in range(1,exp_no+1)], 100-percentile) for t in range(layers_no)]
                    data_en_min_plot[p,k_ind] = [np.percentile([data_en_mean[p,i,k_ind][t] for i in range(1,exp_no+1)], percentile) for t in range(layers_no)]
                else:
    ...
    # y_en_down = {}
        ...
        for j in range(len(noises)):
            ...
            ax[cit,m].fill_between(x[::skip], y_en_up[::skip], y_en_down[::skip], alpha=0.3, color = list_color[j])
...
for nse, noise in enumerate(noise_lebel):
    ax[0,nse].set_title(f'{noise}', fontsize = 10)

for cit in range(len(city_no)):
    for m in range(len(noise_model_str)):
        ax[cit,m].plot(x,y, 'k--', linewidth=.7)
...
fig.text(0.02, 0.5, '$\\frac{|E-E_{\\mathrm{no-mid}}| - |E-E_{\\rm mid}|}{|E-E_{\\mathrm{no-mid}}| + |E-E_{\\rm mid}|}$', va='center', rotation='vertical', fontsize = 12)
# fig.tight_layout()
