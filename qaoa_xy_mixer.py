...
def classical_post_selection_routes(n, dm):
    from math import factorial
    filter_state = np.zeros(2**(n**2))
    for perm in it.permutations(range(n)):
        state = ""
        for i in perm:
            state = state + "0"*i + "1" + "0" *(n-i-1)
        filter_state[int(state, base=2)] = 1
    assert sum(filter_state) == factorial(n)
    
    diag_dm = np.diag(dm)
    return np.multiply(filter_state, diag_dm)

def qaoa_xy(angles, noise_model, diag_vals, midmeasure = None, scheme=None, hamiltonian=None, normalization=None, prob_normalize=None, prob_type=None):
    if normalization == None:
        normalization = "full"
    if prob_normalize == None:
        prob_normalize = False
    if prob_type == None:
        prob_type = "optimal"
    ...
    if prob_type == "optimal":
        min_en_index = np.argmin(diag_vals)
        succ_prob = diag_dm[min_en_index] # success probability according to normalized state
    elif prob_type == "feasible":
        prob_diag_dm = classical_post_selection_routes(n, dm)
        succ_prob = np.sum(prob_diag_dm)
    
    if prob_normalize:
        succ_prob = succ_prob/prob
