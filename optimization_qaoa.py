...
def f(m):
    ...
    f_pure = lambda x: np.real(qaoa_xy(x, noise.NoiseModel(), diag_vals, hamiltonian=hamildict, normalization="shift")[0])
    ...
    t1 = time()
    res_pure = trajectory_optimizer(angles, f_pure)
    print("pure done ({}, {}, {})".format(m, res_pure.fun, time()-t1))
    ...
    np.save(filename + "_pure", res_pure.x)
